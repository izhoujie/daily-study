 ...
 *         手动实现优先于自动绑定 --直接在实现类上加@Singleton可指定该实现类是单例的，同样可在Module时另作变更
 *
 */
public class HelloWorldTest {
	...
	public void test1() {
		Injector injector = Guice.createInjector(new Module() {
			...
			public void configure(Binder binder) {
				// TODO Auto-generated method stub
				binder.bind(HelloWorld.class).toInstance(
						new HelloworldSubImpl());
			}
		});
	...
	public void test4() {
		Injector injector = Guice.createInjector(new Module() {
			...
			public void configure(Binder binder) {
				// TODO Auto-generated method stub
				binder.bind(HelloWorld.class).to(HelloworldSubImpl.class)
						.in(Scopes.SINGLETON);
			}
		});
